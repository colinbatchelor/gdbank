...
\noindent {\bf 6.} Number each of the tokens in the sentence with an integer starting at 0.
...
\noindent {\bf 5.} The example dependencies in these guidelines have no numbers on them. In the actual annotation process you need to number the tokens. The dependencies can be written either with nouns and numbers, separated by an underscore, or numbers on their own.
...
\noindent {\bf 55.} In the phrase \textit{'S e tidsear a th' annam}, \textit{th'} is the \texttt{xcomp} of \textit{tidsear}, \textit{a} is the \texttt{rel} of \textit{th'} and \textit{annam} is an \texttt{adpmod} of \textit{th'}.
...
\noindent{\bf 67.} Mark punctuation with \texttt{p} and the head as the verb head of the preceding phrase. In short sentences this is likely to be the root.
...
\noindent {\bf 22.} Preverbal particles, for example \textit{dh'}, that indicate the type of the sentence take the type \texttt{S[*]/S[dep]}, where * is the relevant one of the foregoing. Tag the main verb with S[dep] and the relevant arguments.

\noindent {\bf 23.} The past-tense marker takes type S[dep]/S[dep] if following a preverbal particle: \textit{Cha do rinn .}
...
\noindent{\bf 47.} \textit{Rach} introducing a passive is \texttt{S[*]/N/S[inf]}.
