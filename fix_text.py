...
with open(sys.argv[2],'w') as fixed:
    for sentence in corpus:
        ...
        fixed.write('\n')
        text = " ".join([t.form for t in sentence]).replace("h- ", "h-").replace("t- ", "t-").replace("n- ", "n-").replace(" ,", ",").replace(" .", ".").replace(" ?", "?").replace("( ", "(").replace(" )",")")
        fixed.write('# text = %s\n' % text)
        ...
        for token in sentence:
            ...
            if token.form == "h-" or token.form == "t-" or token.form =="n-" or token.form == "(" or token.xpos == "Fq":
